...
function Blog({ blogs, color }) {
    return (
        <div className={`grid-container lg:grid-cols-3 sm:grid-cols-1 md:grid-cols-2 '`}>
            {blogs.map(blog => (
                <a href={blog.url}>
                    <div key={blog.id} className={`group bg-${color}-100 rounded-lg shadow-lg hover:shadow-xl hover:scale-105 transition duration-300 ease-in-out overflow-hidden  `}>
                        ...
                        <div className="p-4 flex flex-col justify-between min-h-[344px]">
                            ...
                            <div className="">
                                ...
                                <a href={blog.url} className="read-more text-gray-500 hover:text-gray-700">Read more</a>
                            </div>
